...
class HighScoreRepositoryTest {
    ...
    private fun repo(): HighScoreRepository =
        HighScoreRepository(FileStorage(File(folder.root, "scores.json")))
    ...
    fun `a corrupt table is treated as an empty one`() {
        ...
        assertTrue(HighScoreRepository(FileStorage(file)).load().isEmpty)
    }
    ...
    fun `a table stored out of order is sorted on the way in`() {
        ...
        assertEquals(listOf(900, 400, 100), HighScoreRepository(FileStorage(file)).load().entries.map { it.points })
    }
